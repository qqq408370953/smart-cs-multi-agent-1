...
> 本文负责编排“学习顺序和练习”；术语原理、一次请求十五步、State 演变和各 Agent 的逐步说明见[Node.js Agent 全流程详细讲解](./node-agent-detailed-guide.md)。阅读源码时两份文档应配合使用。

这份路线面向第一次系统学习Agent工程的开发者。建议严格按顺序阅读和动手，不要一开始就接真实LLM或改编排框架。

你也可以先运行`agent-flow-demo/`，点击“学习计划 / 8课”或每一步右侧的“讲解”按钮，在弹框中对照执行说明、输入输出、当前数据、关键代码与准确行号，再回到源码定位对应实现。
